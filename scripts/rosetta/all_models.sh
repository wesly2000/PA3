...
prepare_protocol_datasets() {
    ...
    rosetta_workspace=$(workspace_path "$protocol" "$model_feature" "rosetta")
    ...
    copy_npz_if_missing "$(feature_path "$protocol" "$file_feature" "rosetta")" "${rosetta_workspace}.npz"

    python exp/dataset_process/dataset_merge.py \
        -i "${base_workspace}.npz" "${rosetta_workspace}.npz" \
        -o "${merge_workspace}.npz"

    split_dataset "$base_workspace" "$split_feature"
    split_dataset "$rosetta_workspace" "$split_feature"
...
run_protocol_case() {
    ...
    pretrain_workspace=$(workspace_path "$train_protocol" "$model_feature" "rosetta")
...
cleanup_protocol_datasets() {
    model_feature=$1
    for protocol in "${protocols[@]}"; do
        for suffix in "" "rosetta" "merge"; do
