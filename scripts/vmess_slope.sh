...
MODEL_CONFIGS=(
    ...
    "NetCLR|size_bin|size|size"          # NetCLR + size_bin
)
...
for _cfg_row in "${MODEL_CONFIGS[@]}"; do
    ...
    python exp/dataset_process/dataset_merge.py -i "${root_dir}/workspace/vmess_${model_feature}_bs_filter_strip_${coverage}.npz" "${root_dir}/workspace/vmess_${model_feature}_bs_filter_strip_${coverage}_aug_slope_shadowsocks.npz" "${root_dir}/workspace/vmess_${model_feature}_bs_filter_strip_${coverage}_aug_slope_trojan.npz" -o "${root_dir}/workspace/vmess_${model_feature}_bs_filter_strip_${coverage}_aug_slope.npz"
    ...
    if [[ "${model}" = "NetCLR" ]]; then 
        for ((i=1; i<=iter_num; i++)); do  # NetCLR requires pretraining
            ...
            ./scripts/${model}.sh ${root_dir}/workspace/vmess_${model_feature}_bs_filter_strip_${coverage}_aug_slope ${model_feature} cuda:0 ${root_dir}/workspace/vmess_${model_feature}_bs_filter_strip_${coverage}
