...
prepare_protocol_datasets() {
    ...
    base_workspace=$(workspace_path "$protocol" "$model_feature" "pa3_no_POT")
    pa3_workspace=$(workspace_path "$protocol" "$model_feature" "pa3")
    merge_workspace=$(workspace_path "$protocol" "$model_feature" "merge")

    copy_npz_if_missing "$(feature_path "$protocol" "$file_feature" "pa3_no_POT")" "${base_workspace}.npz"
    copy_npz_if_missing "$(feature_path "$protocol" "$file_feature" "pa3")" "${pa3_workspace}.npz"

    python exp/dataset_process/dataset_merge.py \
        -i "${base_workspace}.npz" "${pa3_workspace}.npz" \
        -o "${merge_workspace}.npz"

    split_dataset "$base_workspace" "$split_feature"
    split_dataset "$pa3_workspace" "$split_feature"
    split_dataset "$merge_workspace" "$split_feature"
}
...
run_protocol_case() {
    ...
    model_feature=$2
    train_protocol=$3
    train_workspace=$(workspace_path "$train_protocol" "$model_feature" "merge")
    pretrain_workspace=$(workspace_path "$train_protocol" "$model_feature" "pa3")
    ...
    while [ "$i" -le "$iter_num" ]; do
        ...
        while IFS= read -r test_protocol; do
            test_workspace=$(workspace_path "$test_protocol" "$model_feature" "pa3_no_POT")
...
cleanup_protocol_datasets() {
    model_feature=$1
    for protocol in "${protocols[@]}"; do
        for suffix in "pa3_no_POT" "pa3" "merge"; do
            workspace=$(workspace_path "$protocol" "$model_feature" "$suffix")
