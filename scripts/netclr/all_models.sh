...
prepare_protocol_datasets() {
    ...
    netclr_workspace=$(workspace_path "$protocol" "$model_feature" "netclr")
    ...
    copy_npz_if_missing "$(feature_path "$protocol" "$file_feature" "netclr")" "${netclr_workspace}.npz"

    python exp/dataset_process/dataset_merge.py \
        -i "${base_workspace}.npz" "${netclr_workspace}.npz" \
        -o "${merge_workspace}.npz"

    split_dataset "$base_workspace" "$split_feature"
    split_dataset "$netclr_workspace" "$split_feature"
...
run_protocol_case() {
    ...
    pretrain_workspace=$(workspace_path "$train_protocol" "$model_feature" "netclr")
