"""
This script is used to compute the frame index of Server Hello packet within a TCP stream.
"""
import os
...
from WFlib.tools.analyzer import PROTOCOL_SH_SEARCHER
...
def pcap_SH_search(pcap_file: str, protocol: str, tshark_path: str, override_prefs) -> List[int]:
    """
    A .pcap file usually contains multiple TCP streams, thus we perform pcap-level SH search following steps:
    ...
    3. For each capture, perform SH search.
    4. Return the frame index of Server Hello packet within each stream.
    ...
    for tcp_stream_number in tcp_stream_numbers:
        with pyshark.FileCapture(pcap_file, 
                                 ...
                                 override_prefs=override_prefs,
                                 custom_parameters=custom_parameters) as cap:
            sh_searcher = PROTOCOL_SH_SEARCHER[protocol]
            sh_indices.append(sh_searcher.search(cap))

    return sh_indices


def host_SH_search(root_dir: str, protocol: str, host: str, tshark_path: str) -> List[int]:
    sh_indices = []
    ...
    for file in sorted([f for f in pcap_dir_path.iterdir() 
        ...
        try:
            sh_indices += pcap_SH_search(file, protocol, tshark_path, override_prefs)
...
def main(input_root: str, output_root: str, host_list_file: str):
    ...
    database_file = f"{output_root}/sh_search/database.csv"
    Path(database_file).parent.mkdir(parents=True, exist_ok=True)

    if not Path(database_file).exists():
        logger.info("Server Hello index database does not exist, create a new one")
        df = pd.DataFrame(columns=PROTOCOLS)
        df.to_csv(database_file, index=False)

    host_list = read_host_list(host_list_file)
    for host in host_list:
        sh_indices = {protocol: [] for protocol in PROTOCOLS}
        for protocol in PROTOCOLS:
            tshark_path = get_tshark_path(config_path, protocol)
            try:
                sh_indices[protocol] = host_SH_search(input_root, protocol, host, tshark_path)
            except Exception as e:
                logger.error(f"Error in host: {host}, Protocol: {protocol}: {e}")
                continue

        # Pad each key's array to the largest length
        max_length = max(len(indices) for indices in sh_indices.values())
        for protocol, indices in sh_indices.items():
            sh_indices[protocol] = indices + [PADDING] * (max_length - len(indices))

        df = pd.DataFrame(sh_indices)
...
if __name__ == "__main__":
    ...
    logger.info("Task sh_search started")
    main(args.input_root, args.output_root, args.host)
    logger.info("Task sh_search completed")