...
def generate_byte_stream(segment_byte_map: dict, cutoff: int) -> np.ndarray:
        """
        ...
        for rel_idx, seg_idx in enumerate(sorted(segment_byte_map.keys())):
            for cover in segment_byte_map[seg_idx]:
                byte_segment[cover[0] : min(cutoff, cover[1])] = rel_idx
...
def generate_byte_segment(lines: list[Line]) -> List[np.ndarray]:
    ...
    return [generate_byte_stream(line.upper_abs_byte_map, min_byte_count) for line in filtered_lines]
